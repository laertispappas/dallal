...
describe UserNotification::Events::Observer do
  subject { UserNotification::Events::Observer }
  it { expect(UserNotification::Events::Observer.callbacks).to eq [] }
  describe '.inherited' do
    context "when a class inherites from observer" do
      let(:klass) do
        class DummyNotifier < UserNotification::Events::Observer
        end
      end

      it 'should validate that model exists and include the model name in NOTIFABLES' do
        expect(UserNotification::Events::Observer).to receive(:validate_model_exists!).with('Dummy').and_call_original
        klass
        expect(UserNotification::Events::Observer::NOTIFIABLES).to include('Dummy')
      end
    end
  end
  ...
  describe '.create_notification' do
    ...
    context 'when no callbacks are available' do
      ...
      it 'does not sipatch any notification' do
        expect_any_instance_of(UserNotification::Notification).to_not receive(:dispatch!)
    ...
    context 'when a callbacks is defined' do
      context "create event" do
        ...
        it 'dipatches a notification' do
          ...
          notification = double('UserNotification::Notification')
          expect(UserNotification::Notification).to receive(:new).with(event: :create, model_class: 'User', opts: { a: 1, b: 2 }, _object: @user).and_return(notification)
      ...
      context "update event" do
        ...
        it 'dispatches an update event' do
          ...
          notification = double('UserNotification::Notification')
          expect(UserNotification::Notification).to receive(:new).with(event: :update, model_class: 'User', opts: { a: 1, b: 2 }, _object: @user).and_return(notification)
          expect(notification).to receive(:dispatch!)
